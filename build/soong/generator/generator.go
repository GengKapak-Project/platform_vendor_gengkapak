...
func init() {
	android.RegisterModuleType("gengkapak_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = gengkapakExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = gengkapakExpandVariables(ctx, dep_file)
	...
	cmd := gengkapakExpandVariables(ctx, String(g.properties.Cmd))
